---
name: plan-writer
description: 基于客户需求生成专业解决方案，采用"草案 + 审阅修订"机制，兼顾方案质量与生成性能
metadata:
...
**降级规则**：若用户消息意图为"更新方案"但 `solution.md` 不存在或为空，静默降级为 Mode A，不向用户确认。
...
```
Phase 1（内联）→ Phase 2: build-plan（1 Agent，含 KB 匹配）→ Phase 3: critique-and-revise（1 Agent）→ Phase 4（内联）
```
...
#### 5.2 Phase 2：方案草案生成（1 Agent，含知识库匹配）
...
- Agent 将草案写入 `docs/plan/draft.md`，对话中只输出 1 句完成状态
- 知识库调用内置于 Agent（最多 1 次 API 调用）

#### 5.3 Phase 3：审阅与修订（1 Agent）

读取 `prompts/critique-and-revise.md`，启动 Agent：

| Agent | Prompt 文件 | 注入数据 |
|-------|-------------|----------|
| critique-and-revise | `prompts/critique-and-revise.md` | `{draft_plan}`（Phase 2 输出）+ `{customer_brief}` |

要求：
- 读取 `docs/plan/draft.md`（Phase 2 的草案）
- 从 5 个维度审阅草案（需求理解、方案规模、成本预算、时间资源、技术可行性）
- 只修改有实质性问题的章节，无问题章节保留原文
- Agent 将最终方案（含 YAML frontmatter）写入 `docs/plan/solution.md`，对话中只输出 1 句完成状态

#### 5.4 Phase 4：局部 humanizer + 输出状态

Phase 3 的 critique-and-revise Agent 已将最终方案写入 `docs/plan/solution.md`。

1. 读取 `docs/plan/solution.md`，提取"客户概况"和"整体思路"等叙述性段落，交给 `humanizer-zh` 做局部风格优化。若不可用则跳过。优化后写回 `docs/plan/solution.md`。

2. 删除中间文件 `docs/plan/draft.md`。

3. 向用户输出 1-2 句完成状态，包括方案版本号和写入文件路径。不输出方案内容本身。
...
后续 Phase 2 / 3 / 4 复用 Mode A 流程：
- `build-plan` 以 `{customer_brief}` 为输入
- `critique-and-revise` 审阅修订
- 局部 humanizer
...
- **大改动**（需求方向变化、关键目标/约束大幅调整）：
  - 重新执行 Mode A 的 Phase 1–4，生成全新方案版本

写入规则与 Mode A Phase 4 一致，版本号按以下规则递增：
...
└── prompts/
    ...
    ├── build-plan.md              # 方案草案生成（内置 KB 匹配，直接输出 Markdown）
    └── critique-and-revise.md     # 审阅与修订（合并批判+修订，直接输出 Markdown）
```
